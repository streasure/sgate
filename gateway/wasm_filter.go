...
func (w *WasmFilter) Phase() FilterPhase { return PhasePostAuth }
func (w *WasmFilter) Priority() int      { return 250 }

// Process 把请求交给 WASM 插件处理
// 返回码：0 = 放行；非 0 = 中止
func (w *WasmFilter) Process(fc *FilterContext) (bool, error) {
...
func buildWasmInput(fc *FilterContext) []byte {
...
func applyWasmOutput(fc *FilterContext, out []byte) {
...
func init() {
	RegisterFilter("wasm-filter", func(cfg map[string]interface{}) (Filter, error) {
		return NewWasmFilter(getString(cfg, "module"), getString(cfg, "function"))
	})
}
