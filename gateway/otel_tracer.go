...
func NewOTelTracer(cfg OTelTracerConfig) *OTelTracer {
	t := &OTelTracer{
		endpoint:      cfg.Endpoint,
		serviceName:   cfg.ServiceName,
		localEndpoint: map[string]string{"serviceName": cfg.ServiceName},
		httpClient:    &http.Client{Timeout: 3 * time.Second},
		queue:         make(chan *OTelSpan, maxInt(cfg.QueueSize, 1024)),
...
func (f *OTelSpanFilter) Phase() FilterPhase { return PhaseForward }
func (f *OTelSpanFilter) Priority() int      { return 50 }

func (f *OTelSpanFilter) Process(fc *FilterContext) (bool, error) {
	if f.tracer == nil {
		return true, nil
	}
...
func init() {
	RegisterFilter("otel-tracer", func(cfg map[string]interface{}) (Filter, error) {
		c := OTelTracerConfig{
			Endpoint:    getString(cfg, "endpoint"),
			ServiceName: getString(cfg, "serviceName"),
			SampleRate:  getInt(cfg, "sampleRate"),
