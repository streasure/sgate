...
func (f *EBPFAcceleratorFilter) Phase() FilterPhase { return PhasePreAuth }
func (f *EBPFAcceleratorFilter) Priority() int      { return 10 } // 最先执行

func (f *EBPFAcceleratorFilter) Process(fc *FilterContext) (bool, error) {
	if f.hook == nil {
		return true, nil
	}
...
func init() {
	RegisterFilter("ebpf-accelerator", func(cfg map[string]interface{}) (Filter, error) {
		hook := GetKernelHook()
