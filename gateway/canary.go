...
func NewCanaryFilter(cfg CanaryConfig) *CanaryFilter {
	f := &CanaryFilter{
		percent:     cfg.Percent,
		headers:     cfg.Headers,
...
func (f *CanaryFilter) Phase() FilterPhase { return PhasePostAuth }
func (f *CanaryFilter) Priority() int      { return 200 }

// Process 判断是否命中灰度
func (f *CanaryFilter) Process(fc *FilterContext) (bool, error) {
	f.mu.RLock()
	defer f.mu.RUnlock()
...
func (f *CanaryFilter) UpdateConfig(cfg CanaryConfig) {
	f.mu.Lock()
	defer f.mu.Unlock()
...
func init() {
	RegisterFilter("canary", func(cfg map[string]interface{}) (Filter, error) {
		c := CanaryConfig{
			Percent:     getInt(cfg, "percent"),
			TargetRoute: getString(cfg, "targetRoute"),
		}
