...
func NewJWTAuthFilter(cfg JWTAuthConfig) *JWTAuthFilter {
	f := &JWTAuthFilter{
...
func (f *JWTAuthFilter) Phase() FilterPhase { return PhaseAuth }
func (f *JWTAuthFilter) Priority() int      { return 100 }

// Process 鉴权处理：从元数据取 token，校验签名+过期+撤销
func (f *JWTAuthFilter) Process(fc *FilterContext) (bool, error) {
...
// init 自动注册到 SPI 注册表
func init() {
	RegisterFilter("jwt-auth", func(cfg map[string]interface{}) (Filter, error) {
		c := JWTAuthConfig{
			Secret:      getString(cfg, "secret"),
			Issuer:      getString(cfg, "issuer"),
			HeaderField: getString(cfg, "headerField"),
		}
		// SkipRoutes
